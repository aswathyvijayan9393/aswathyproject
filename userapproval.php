...
<div class="rowtab"><div class="rowcello"><input name="approve" type="submit" value="approve" /></div></div>
</div>
			 <?php
 if(isset($_POST['approve']))
					{
                        $upd = mysql_query("UPDATE tbllogin SET status='approve' WHERE username='$s[username]'");//update status in login
                   
				    if($upd)
					{
					echo "<script>alert('Approved Successfully');window.location='admin.php';</script>";
					}
					else 
					{
					echo "mysql_error()";
